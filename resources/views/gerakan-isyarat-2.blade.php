...
<body>
    ...
    <script type="module">
        ...
        async function predictGesture(landmarks) {
            try {
                // Flat array: [x1, y1, z1, x2, y2, z2, ..., x21, y21, z21]
                const flat = landmarks.flatMap(p => [p.x, p.y, p.z]);

                // Optional: normalize input (uncomment if your model was trained with normalized data)
                // const mean = tf.mean(flat);
                // const std = tf.moments(flat).variance.sqrt();
                // const normalized = flat.map(v => (v - mean.arraySync()) / std.arraySync());

                if (flat.length !== 63) {
                    console.warn("Invalid input length:", flat.length);
                    gestureText.innerText = "Invalid input";
                    return;
                }

                const input = tf.tensor(flat, [1, 21, 3]);
