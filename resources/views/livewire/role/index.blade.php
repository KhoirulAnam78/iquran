    ...
    <div class="card card-height-100">
        ...
        <div class="card-header border-bottom-dashed align-items-center d-flex">
            ...
            <div class="flex-shrink-0">
                <a href="{{ route('role.create') }}" wire:navigate class="btn btn-soft-primary btn-sm">
                ...
                <table class="table table-nowrap table-striped-columns mb-0">
                    ...
                    <tbody>
                        @forelse ($roles as $role)
                            <tr>
                                <th scope="row">
                                    {{ ($roles->currentpage() - 1) * $roles->perpage() + $loop->index + 1 }}</th>
                                <td>{{ $role->name }}</td>
                                <td>{{ $role->guard_name }}</td>
                                <td>{{ $role->descriptions ?? '-' }}</td>
                                <td>
                                    <a href="{{ route('role.edit', ['id' => Crypt::encryptString($role->id)]) }}"
                                        class="btn btn-primary" wire:navigate><i class="ri-quill-pen-line"></i></a>
                                    <button class="btn btn-danger"
                                        wire:click="showDelete('{{ Crypt::encryptString($role->id) }}')"><i
        ...
        <div class="card-footer py-4 d-flex">
            <p class="flex-grow-1">Total data : {{ $roles->total() }}</p>
