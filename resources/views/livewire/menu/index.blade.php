    ...
    <div class="card card-height-100">
        ...
        <div class="card-header border-bottom-dashed align-items-center d-flex">
            ...
            <div class="flex-shrink-0">
                <a href="{{ route('menu.create') }}" wire:navigate class="btn btn-soft-primary btn-sm">
                ...
                <table class="table table-nowrap table-striped-columns mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th scope="col" wire:click='sortBy("permission")'>Permission <i
                                    class="ri-arrow-down-s-fill"></i></th>
                            <th scope="col" wire:click='sortBy("position")'>Position <i
                                    class="ri-arrow-down-s-fill"></i></th>
                            <th>Icon</th>
                            <th>Is Child?</th>
                            <th>Parent Menu</th>
                    ...
                    <tbody>
                        @forelse ($menus as $menu)
                            <tr>
                                <th scope="row">
                                    {{ ($menus->currentpage() - 1) * $menus->perpage() + $loop->index + 1 }}</th>
                                <td>{{ $menu->name }}</td>
                                <td>{{ $menu->permission }}</td>
                                <td>{{ $menu->position }}</td>
                                <td><i class="{{ $menu->icon }}"></i></td>
                                <td>{{ $menu->parent_menu == 1 ? 'Yes' : 'No' }}</td>
                                <td>{{ $menu->parent->name ?? 'Nothing' }}</td>
                                <td>
                                    {{ $menu->status == 1 ? 'Show' : 'Hide' }}
                                </td>
                                <td>{{ $menu->descriptions }}</td>
                                <td>
                                    <a href="{{ route('menu.edit', ['id' => Crypt::encryptString($menu->id)]) }}"
                                        class="btn btn-primary" wire:navigate><i class="ri-quill-pen-line"></i></a>
                                    <button class="btn btn-danger"
                                        wire:click="showDelete('{{ Crypt::encryptString($menu->id) }}')"><i
        ...
        <div class="card-footer py-4 d-flex" style="font-size:12px">
            <p class="flex-grow-1">Total data : {{ $menus->total() }}</p>
